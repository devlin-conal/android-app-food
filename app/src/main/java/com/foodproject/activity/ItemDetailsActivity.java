...
public class ItemDetailsActivity extends AppCompatActivity implements ExtrasAdapter.OnExtraClickListener {
    ...
    private List<IngredientResponse> listIngredients = new ArrayList<>();
    ...
    private void initComponents() {
        ...
        mItemDesc = findViewById(R.id.item_description);
        ...
        if (menu != null) {
            array_imgs = new String[listIngredients.size() + 1];
            array_imgs[0] = menu.getThumbnailPic();
            for (int i = 0; i < listIngredients.size(); i++) {
                array_imgs[i + 1] = listIngredients.get(i).getThumbnailPic();
            }

            mItemName.setText(menu.getName() + menu.getId());
            mItemDesc.setText(menu.getDescription());
        }
    ...
    private void getArgs() {
        Intent i = getIntent();
        menu = (MenuResponse) i.getSerializableExtra(ARG_MENU);
        listIngredients.addAll(menu.getIngredients());
    }
    ...
    private void setupWidgets() {
        ...
        if (menu != null) {
            mAdapter.setmExtras(listIngredients);
        }
        ...
        mPlace.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(ItemDetailsActivity.this, RestaurantActivity.class);
                startActivity(i);
            }
        });
    ...
    @Override
    public void onExtraClickListener(IngredientResponse extra) {
...
//            AndroidUtil.displayImageIntoSlider(act, imageView, array_imgs, position);
            Glide.with(act)
                    .load(Headers.getUrlWithHeaders(array_imgs[position], sharedPrefManager.getSPToken()))
                    .into(imageView);
